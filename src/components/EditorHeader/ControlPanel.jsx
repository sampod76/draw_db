...
import { dataURItoBlob, getTableHeight } from "../../utils/utils";
...
export default function ControlPanel({
        ...
        if (a.component === "field") {
          ...
          updateTable(a.tid, {
            indices: table.indices.map((index) =>
              index.id === a.iid ? { ...index, ...a.undo } : index,
            ),
          });
  ...
  const redo = () => {
    ...
    if (a.action === Action.ADD) {
      if (a.element === ObjectType.TABLE) addTable(null, false);
      else if (a.element === ObjectType.AREA) addArea(null, false);
      else if (a.element === ObjectType.NOTE) addNote(null, false);
      else if (a.element === ObjectType.RELATIONSHIP)
        addRelationship(a.data, false);
      else if (a.element === ObjectType.TYPE) addType(null, false);
      else if (a.element === ObjectType.ENUM) addEnum(null, false);
      setUndoStack((prev) => [...prev, a]);
    } else if (a.action === Action.MOVE) {
      if (a.element === ObjectType.TABLE) {
        const { x, y } = tables.find((t) => t.id == a.id);
      ...
      if (a.element === ObjectType.TABLE) deleteTable(a.data.table.id, false);
      else if (a.element === ObjectType.RELATIONSHIP)
        deleteRelationship(a.data.id, false);
      else if (a.element === ObjectType.NOTE) deleteNote(a.data.id, false);
      else if (a.element === ObjectType.AREA) deleteArea(a.data.id, false);
      else if (a.element === ObjectType.TYPE) deleteType(a.id, false);
      ...
      if (a.element === ObjectType.AREA) {
        ...
        if (a.component === "field") {
          ...
          updateTable(a.tid, {
            indices: table.indices.map((index) =>
              index.id === a.iid ? { ...index, ...a.redo } : index,
            ),
          });
        } else if (a.component === "index_delete") {
          updateTable(a.tid, {
        ...
        if (a.component === "field_add") {
          updateType(a.tid, {
            fields: [...types[a.tid].fields, { name: "", type: "" }],
          });
          ...
          if (a.updatedFields && a.redo.name) {
            a.updatedFields.forEach((x) =>
  ...
  const paste = () => {
    navigator.clipboard.readText().then((text) => {
      ...
      if (v.validate(obj, tableSchema).valid) {
        addTable({ ...obj, x: obj.x + 20, y: obj.y + 20, id: nanoid() });
  ...
  const menu = {
    ...
    edit: {
      ...
      duplicate: { function: duplicate, shortcut: "Ctrl+D" },
      delete: { function: del, shortcut: "Del" },
      copy_as_image: { function: copyAsImage, shortcut: "Ctrl+Alt+C" },
    },
    view: {
      header: {
        state: layout.header ? (
      ...
      reset_view: { function: resetView, shortcut: "Enter/Return" },
      show_datatype: {
    ...
    settings: {
      ...
      export_saved_data: { function: exportSavedData },
      flush_storage: {
        warning: {
    ...
    help: {
      ...
      report_bug: { function: () => window.open("/bug-report", "_blank") },
    },
  };
